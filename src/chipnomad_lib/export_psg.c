...
static void psgChipSetRegister(struct SoundChip* self, uint16_t reg, uint8_t value) {
    ...
    if (data->lastRegs[reg] != value || reg == 13) {
        // Mark frame start if this is the first register write in this frame
        if (!data->frameStarted) {
            uint8_t frameMarker = 0xFF;
            fileWrite(data->exporter->fileId, &frameMarker, 1);
            data->frameStarted = 1;
        }
...
int exportProjectToPSG(const char* filename, struct Project* project, int startRow) {
    ...
    do {
        // Reset frame marker for new frame
        PSGChipData* chipData = (PSGChipData*)chip.userdata;
        chipData->frameStarted = 0;

        allTracksStopped = playbackNextFrame(&playbackState, &chip);
        exporter->currentTick++;
